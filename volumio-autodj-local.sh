#!/usr/bin/env bash
# =============================================================================
# NOTE: this is a bundled copy of volumio-autodj-local.sh from the sibling
# https://github.com/Celindir69/volumio-autodj repository, invoked by this
# plugin's index.js on a timer with VOLUMIO_HOST/LASTFM_API_KEY/
# ARTIST_HISTORY_SIZE set from the plugin's own settings (see saveSettings()
# and runTick() in index.js). Keep the two files in sync when changing the
# AutoDJ logic
...
# artist one. Unless NONE of the current candidates survive the artist
# guard, in which case it's overridden as a fallback (see step 4 below)
# rather than leaving the queue to run dry.
# =============================================================================
set -euo pipefail
...
# In "--watch-boundary" mode, the queue position is polled via curl/jq
# against Volumio's own REST API (needed - see the comment above
# boundary_watch_tick(): MPD's own raw position isn't reliable when it runs
# in consume mode), and crossfade still goes through mpd_raw_query() (bash's
# own /dev/tcp, no external tool). "mpc" is the one tool NOT needed in that
# mode unless AUTO_REPLAYGAIN is actually on (replaygain still goes through
# "mpc replaygain" - see the comment above replaygain_query()); skip
